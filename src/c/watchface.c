...
static int battery_percent, step_goal=10000, line_width=4;
...
static void health_update_proc(Layer *layer, GContext *ctx) {
  ...
  graphics_draw_line(ctx, GPoint(0, bounds.size.h-line_width), GPoint((step_count/step_goal)*bounds.size.w, bounds.size.h-line_width));
}
